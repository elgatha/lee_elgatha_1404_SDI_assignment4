...
//Phone Number
//Check for (XXX-XXX-XXXX) Number format
...
var checkPhoneNumber = function(val) {
	...
	var dash2 = (phoneNumber.substring(7,8));
		...
		if (dash1 == "-" && dash2 == "-") {
			...
			return false;
			console.log("NOPE! Does not compute! Please try again ");
		}
...
var checkEmail = function(val) {
	...
	if (at == -1 || dot == -1 || (at + 2) >= dot) {
		return false; 
		console.log("NOPE! Does not compute!");
	}else {
		return true;
		console.log("BOOM! You've entered your eMail correctly!   Congrats!");
	}
};
...
var chedkURL = function(val) {
	...
	if (at != -1 || https != -1) {
		return true; 
		console.log("BOOM! You've entered your URL correctly! 2 points!");
	}else {
		return false;
		console.log("NOPE! Does not compute!");
...
// Output format [Days] conversion for each at specific index shown
